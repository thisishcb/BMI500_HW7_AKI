-- Adopted from https://github.com/MIT-LCP/mimic-code/blob/main/mimic-iii/concepts/organfailure/kdigo_uo.sql

with ur_stg as
(
  select io.SUBJECT_ID, io.HADM_ID, io.icustay_id, io.charttime
  ...
  from `mimiciii-328704.mimicself.urine_output` io
  -- this join gives you all UO measurements over a 24 hour period
  left join `mimiciii-328704.mimicself.urine_output` iosum
    on  io.SUBJECT_ID = iosum.SUBJECT_ID
    and io.charttime >= iosum.charttime
    and io.charttime <= (DATETIME_ADD(iosum.charttime, INTERVAL '23' HOUR))
  group by io.SUBJECT_ID, io.HADM_ID, io.icustay_id, io.charttime
)
